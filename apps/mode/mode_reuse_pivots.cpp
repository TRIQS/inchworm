        ...
        for (auto [iota_d_list, iota_d_dag_list] : iota_pair_list) {
          ...
          if (tp.tci_prrlu) {
            ...
            if (previous_pivots.size() > 0) {
              ...
              // integral_element = ci.tt.sum(std::vector(n,  tp.wi_v));
              int temp_bound = 1;
              for (int i = 0; i < temp_bound; i++) {
                auto lastPivotError = ci.pivotError[ci.pivotError.size() - 1];
                if (lastPivotError >  tp.error_bound) {
                  ci.iterate();
                  ci.makeCanonical();
                }
                lastPivotError = ci.pivotError[ci.pivotError.size() - 1];
                current_integral    = ci.tt.sum(std::vector(n,  tp.wi_v));
                if (sp.debug) { std::cout << i << " " << count << " " << lastPivotError << " " << current_integral << std::endl; }
                if (std::abs(current_integral - previous_integral) <  tp.error_bound && i > 0) { break; }
