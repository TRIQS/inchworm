...
void ModePartitionMid::run_single_element() {
  ...
  for (int order : sp.order_list) {
    ...
    for (auto [phi_d_list, phi_d_dag_list] : phi_pair_list) {
      ...
      long count                 = 0;
      auto get_u_tau_max_element = [this, &count, &n, &phi_d_list = phi_d_list,
                                    &phi_d_dag_list = phi_d_dag_list](const std::vector<double> &fulls) -> double {
        ...
        std::vector<int> iota_list(fulls.begin() + 1, fulls.begin() + 1 + n);
        ...
        std::vector<double> vs(fulls.begin() + 1 + n, fulls.end());
...
#pragma omp critical
      ...
      int max_size        = 20;
      if (further_reduce) {
        if (valid_pivots.size() > max_size) { valid_pivots.resize(max_size); }
      }
      pivot1 = valid_pivots[0];
      // for(int i = 0; i < valid_pivots.size(); i++){
      //   std::cout << "pivot1 = ";
      //   print_vector(valid_pivots[i]);
      //   std::cout << std::endl;
      //   std::cout << "value  ="<< valid_pivots_value[i] << std::endl;
      // }
      ...
      //   std::cout << "\n";
      // }
      // std::cout << "valid_pivots= ";
      // for (auto i : valid_pivots) {
      //   print_vector(i);
      //   std::cout << "\n";
      // }
