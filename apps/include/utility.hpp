...
inline double evaluate_diagram(frame_t &frame_zeroth_order, double tau_split, double tau_max, std::vector<std::vector<fop_t>> const &all_d_ops,
  ...
  if (gf_index.size() == 0) { // partiion function or propagator
    if (!use_bare_propagator) {
      ...
      NVTX_RANGE("bare-impurity product", 5)
...
inline bool if_contains(const std::vector<double> &taus, double tau) { return std::find(taus.begin(), taus.end(), tau) != taus.end(); }
...
do_TCI(int rank, const std::string &debug_info, std::function<T_output(std::vector<T_input>)> integrand,
  ...
  if (tci_prrlu == 2 && adaptive_error) {
    ...
    for (int i = 1; i <= sweep_bound; i++) {
      ...
      if (adaptive_error) {
        if (i != 1) {
          if (*ci.auxi_height == ci.pivotError[ci.pivotError.size() - 1] * decay_rate || *ci.auxi_height < 1e-15 || i==sweep_bound) {
      ...
      if (debug > 1 || i == sweep_bound) {
        ...
        T_output diff = 0;
        bool all_zero = true;
        for (auto jj = 0u; jj < integral.size(); jj++) {
          diff += std::abs(previous_integral[jj] - integral[jj]);
          double TOL = 1e-14;
          if (abs(integral[jj]) > TOL || abs(previous_integral[jj]) > TOL) { all_zero = false; }
        }
        if (i > 5 && all_zero) { return integral; }
    ...
    if(valid_init_global_pivot.size()==0){
      std::vector<T_output> empty_integral{};
      if (unsummed_tci != 0) {
        empty_integral          = std::vector<T_output>(unsummed_tot_size, 0);
      } else {
        empty_integral          = std::vector<T_output>(1, 0);
      }
      return empty_integral;
    }
    // ci.makeCanonical();
