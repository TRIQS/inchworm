...
void ModeBase::read_json_parameters(const std::string &json_file_path) {
  ...
  //// required parameters
  try{
  gp.read_general_params = root.get<bool>("gp.read_general_params");}
  catch (const std::exception &e) { gp.read_general_params = false; }
  if (gp.read_general_params) {
    try {
      gp.hopping_file_path     = root.get<std::string>("gp.hopping_file_path");
      gp.interaction_file_path = root.get<std::string>("gp.interaction_file_path");
    } catch (const std::exception &e) { throw std::runtime_error("Error: hopping_file_path or interaction_file_path is not provided"); }
  }
  ...
  } catch (const std::exception &e) { gp.hyb_file_path = ""; }
  try {
    gp.propagator_file_path = root.get<std::string>("gp.propagator_file_path");
  } catch (const std::exception &e) { gp.propagator_file_path = ""; }
  //// optional parameters
  try {
...
hyb_tau_t ModeBase::read_hyb_function() {
  ...
  // check if the hyb_file_path ends with .h5
  if (gp.hyb_file_path.substr(gp.hyb_file_path.size() - 3) == ".h5") {
    ...
    if(tau_grid.size() != cp.n_tau_hyb) {
      throw std::runtime_error("Error[Params::readDeltaFile]: tau_grid size in hyb_file_path does not match cp.n_tau_hyb");
    }
    ...
    for (int block = 0; block < cp.gf_struct.size(); block++) {
      for (auto [i, j] : product_range(mp.n_site, mp.n_site)) {
        ...
        for (int k = 0; k < Delta_tau_grid.size(); k++) { Delta_tau[block][k](i, j) = Delta_tau_ij[k]; }
      }
    }

    std::cout << "Read hybridization function from file: " << gp.hyb_file_path << std::endl;
    
    return Delta_tau;
  }
  else if (gp.hyb_file_path.substr(gp.hyb_file_path.size() - 4) == ".txt") {
    auto Delta_tau      = hyb_tau_t{{cp.beta, Fermion, static_cast<long>(cp.n_tau_hyb)}, cp.gf_struct};
    std::ifstream file(gp.hyb_file_path);
    if (!file.is_open()) {
      throw std::runtime_error("Error[Params::readDeltaFile]: Could not open file " + gp.hyb_file_path);
    }
    else{
      std::cout << "reading delta from file: " << gp.hyb_file_path << std::endl;
    }
    size_t n_flavors = mp.n_site * mp.n_spin;
    size_t tauIdx, flavorI, flavorJ;
    double real, imag;
    for (size_t i = 0; i < cp.n_tau_hyb; ++i) {
      for (size_t j = 0; j < n_flavors; ++j) {
        for (size_t k = 0; k < n_flavors; ++k) {
          file >> tauIdx >> flavorI >> flavorJ >> real >> imag;
          if (tauIdx != i || flavorI != j ||
              flavorJ != k) {
                std::cout << "tauIdx: " << tauIdx << "flavorI: " << flavorI << "flavorJ: " << flavorJ << std::endl;
                std::cout << "i: " << i << "j: " << j << "k: " << k << std::endl;
            throw std::runtime_error("Error[Params::readDeltaFile]: parsing delta file: " + gp.hyb_file_path);
          } // the order of spin-orbit flavor is 0 up, 0 down, 1 up, 1 down, etc
          size_t bl_j = j % mp.n_spin;
          size_t bl_k = k % mp.n_spin;
          if(bl_j == bl_k){
            size_t sub_j = j / mp.n_spin;
            size_t sub_k = k / mp.n_spin;
            Delta_tau[bl_j][i](sub_j, sub_k) = real;
          }
          else{
            if(abs(real) > TOL || abs(imag) > TOL){
              throw std::runtime_error("Error[Params::readDeltaFile]: hybridization function is not block diagonal");
            }
          }
        }
      }
...
void ModeBase::read_propagator() {
  if (gp.propagator_file_path.empty()) { throw std::runtime_error("Error: propagator_file_path is empty"); }
  ...
  if (tau_grid != sp.grid) { throw std::runtime_error("Error: tau_grid in propagator_file_path does not match sp.grid"); }
  ...
  for (int bl = 0; bl < mp.ad_imp.n_subspaces(); bl++) {
    for (int i = 0; i < mp.ad_imp.get_subspace_dim(bl); i++) {
      for (int j = 0; j < mp.ad_imp.get_subspace_dim(bl); j++) {
        ...
        for (int k = 0; k < sp.grid.size(); k++) { sr.u_tau[bl][k](i, j) = u_tau_ij[k]; }
      }
    }
  }
}
