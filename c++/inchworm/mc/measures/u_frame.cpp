...
namespace inchworm::measures {
  ...
  u_frame::u_frame(params_t const &, qmc_config_data_t const &data_, single_step_results_t &results_) : data(data_), results(results_) {}
  ...
  void u_frame::accumulate(scalar_t sign) {
    ...
    auto const &[tbl0, m0] = data.u_partial[0];
    ...
    for (int bl = 0; bl < results.frame.size(); bl++) {
      auto const &[tbl, m] = data.u_partial[bl];
