...
namespace inchworm::moves {
  ...
  scalar_t remove::attempt() {
    ...
    prop_config  = data.config; // we first copy last accepted config before proposing the new remove.
    ...
    if (not prop_config.try_erase(idx, idx_dag)) return 0; //data is not modified in this case
