...
void printArch(int a, int b, int k_order, char char1='.'){
  ...
  for(ii=0; ii<2*k_order; ii++){
    if((ii>=a) and (ii<b)) printf("%c%c",char1,char1);
    else if((ii==b)) printf("%c ",char1);
    else printf("  ");
  }
  printf("\n");
}
...
//         ________
//
bool segment_cross(int a1, int b1, int a2, int b2){
...
//         ________
//
bool segment_cross_point(int a, int b, int point){
  return (a-(point+0.5))*(b-(point+0.5))<0.0;
}
...
//
void print_graph(const std::vector<int> & permutation,const std::vector<bool> & cross_split_point, std::vector<bool> & visited,const std::vector<int> & pos_o,  const std::vector<int> & pos_x, const int k_order, const int split_point, std::string diagram){
...
//
void grow_pile(int arch, std::vector<int> const & permutation, std::vector<bool> & visited, std::vector<int> const & pos_o, std::vector<int> const & pos_x, int k_order){
...
//
bool test_diagram_connection(int split_point, const std::vector<int> & permutation, const std::vector<int> & pos_o,  const std::vector<int> & pos_x, int k_order, std::string diagram, int verbose=0){
  ...
  if(verbose>1)
    print_graph(permutation, cross_split_point, visited, pos_o, pos_x, k_order, split_point, diagram);
...
int find_proper_diagrams(std::string const & diagram, int split_point, int verbose = 0)
{
  ...
  do {
    ...
    if(test_diagram_connection(split_point, permutation, pos_o, pos_x, k_order, diagram, verbose)){
      N_proper+=1;
    }
