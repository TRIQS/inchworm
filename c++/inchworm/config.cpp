...
namespace inchworm {
  ...
  bool config_t::try_double_insert(fop_t const &d_dag1, fop_t const &d1, fop_t const &d_dag2, fop_t const &d2) {
    ...
    if (std::count(split_times.cbegin(), split_times.cend(), d1.tau) > 0) return false;
    if (std::count(split_times.cbegin(), split_times.cend(), d_dag1.tau) > 0) return false;
    if (std::count(split_times.cbegin(), split_times.cend(), d2.tau) > 0) return false;
    if (std::count(split_times.cbegin(), split_times.cend(), d_dag2.tau) > 0) return false;
