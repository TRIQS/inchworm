...
namespace inchworm {
  ...
  inline double cyclic_difference(auto const &op, config_t const &config, double tau_max) {
    if (config.size(op.bl) == 0) TRIQS_RUNTIME_ERROR << "cyclic_difference requires the operator block to no be empty";

    double min_dist = tau_max;
    double res      = tau_max;
    for (auto const &opdiag : op.dag ? config.d_bl_list[op.bl] : config.d_dag_bl_list[op.bl]) {
