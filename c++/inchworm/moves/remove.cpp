...
namespace inchworm::moves {
  ...
  mc_weight_t remove::attempt() {
    ...
    // temporary values:
    new_config = data.config; // we first copy last accepted config before proposing the new remove.
    new_w      = data.w;

    int N     = data.size();
    int i     = rng(N);
    int i_dag = rng(N);
    if (not new_config.try_erase(i, i_dag)) return 0; //data is not modified in this case
