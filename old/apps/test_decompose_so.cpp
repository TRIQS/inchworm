...
int main() {
  ...
  for (int order : order_list) {
    ...
    auto [phi_d_list, phi_d_dag_list] = phi_pair_list[1];
    std::cout << "phi_d_list: ";
    ...
    for (auto [iota_d_list, iota_d_dag_list] : iota_pair_list) {
      ...
      // for (int i = 0; i < iota_d_list.size(); i++) {
      //   outfile << iota_d_list[i];
      //   outfile << iota_d_dag_list[i];
      // }
      outfile << " ";
      outfile << std::fixed << std::setprecision(20) << current_integral << "\n";
