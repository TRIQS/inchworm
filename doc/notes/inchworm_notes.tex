...
\end{align}
where $\tau_k > \cdots >\tau_1$ and $\tau_k^\prime > \cdots >\tau_1^\prime$. This is the normal order, i.e. the order that does not have a sign. Practically, we need to order the operators for all the times togheter such that $\tau_{left} > \cdots >\tau_{right}$ but there is a sign $(-1)^\sigma$ where $\sigma$ is the number of permutation one must do to order all the times starting from the normal order. 
