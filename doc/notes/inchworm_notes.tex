...
\end{align}
where $\tau_k > \cdots >\tau_1$ and $\tau_k^\prime > \cdots >\tau_1^\prime$. This is the normal order, i.e. the order that does not have a sign. Practically, we need to order the operators for all the times togheter such that $\tau_{left} > \cdots >\tau_{right}$. This reordering imply a new sign $(-1)^\sigma$ where $\sigma$ is the number of permutation one must do to order all the times starting from the normal order. 
\begin{align}
U(\beta)=& \sum_k^{\infty}
\int_{0}^{\beta} d \tau_1 \cdots \int_{\tau_{k-1}}^{\beta} d \tau_k
\int_{0}^{\beta} d \tau_1 \cdots \int_{\tau_{k-1}^\prime}^{\beta} d \tau_k^{\prime}
...
U(\beta)=& \frac{1}{Z_{\text {bath }}}\operatorname{Tr_B} [e^{-\beta (H_a + H_\text{hyb}) }]  
\\
U(\beta)=& \frac{1}{Z_{\text {bath }}} \text{Tr}_B\left[
\sum_{k} \frac{1}{k !} 
\sum_{a b} 
e^{-\beta H_\text{loc}}
\int_{0}^{\beta} 
